...
### Race Condition 대시보드 (추가 기능)

Optimistic Locking으로 감지된 버전 충돌을 실시간으로 추적·시각화하는 대시보드입니다. 동시성 문제가 실제로 얼마나, 언제, 누구에 의해 발생하는지 한눈에 확인할 수 있습니다.

#### 주요 지표

| 지표 | 설명 |
|------|------|
| **총 쓰기 요청** | 버전 충돌이 발생할 수 있는 작업(수정·상태변경·이동·순서변경) 누적 횟수 |
| **충돌 횟수** | Optimistic Locking으로 감지된 버전 불일치 누적 횟수 |
| **충돌률** | 충돌 횟수 / 총 쓰기 요청 × 100 (%) |
| **최근 충돌 이력** | 충돌 발생 시 카드명, 작업자 닉네임, 예상/실제 버전, 시각을 최대 20건 기록 |

#### 동작 원리

```
[카드 수정 요청] → version 비교 → 불일치 감지
                                       ↓
                              conflict_count 증가
                              recent_conflicts 기록
                                       ↓
                        emit_ws_only(StatsUpdated) → 모든 클라이언트 실시간 반영
```

- 충돌 이벤트는 WAL에 기록하지 않고 WebSocket으로만 전송 (상태 복구와 무관한 통계 데이터)
- `GET /api/stats` API로 현재 통계 조회 가능
- 웹 UI 상단 **Race 대시보드** 버튼 클릭 시 모달로 확인 가능
- 대시보드가 열려 있는 상태에서 충돌이 발생하면 자동으로 수치가 갱신됨
...
**Race Condition 대시보드 — 버전 충돌 통계 실시간 시각화**
- `ConflictEvent`, `DashboardStats` 모델 설계 — 충돌 발생 시 카드명, 작업자 닉네임, 예상/실제 버전, 시각을 구조화하여 기록
- 충돌 통계 추적 로직 구현 — `update_card` / `update_card_status` / `move_card` / `reorder_card` 각 메서드에 `total_writes` 카운터와 `record_conflict()` 호출 추가
- `emit_ws_only()` 분리 — 통계 이벤트는 WAL에 기록하지 않고 WebSocket으로만 브로드캐스트하여 WAL 오염 방지
- `GET /api/stats` API 추가 — 현재 누적 통계(총 쓰기 요청, 충돌 횟수, 충돌률, 최근 충돌 이력)를 JSON으로 반환
- 웹 UI 대시보드 구현 — 총 쓰기 요청·충돌 횟수·충돌률을 카드로 표시하고, 최근 충돌 이력을 타임라인으로 나열; WebSocket `stats_updated` 이벤트 수신 시 새로고침 없이 자동 갱신

## 👥 팀 구성

| 이름 | 역할 |
|------|------|
| 이동열 | 사용자 닉네임 식별 + 실시간 동기화 + Audit Log 분리 저장 |
| 이재익 | PR: 시스템 아키텍처 설계 |
